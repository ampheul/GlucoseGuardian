...
void ArtificialPancreas::calculateMedication(const double reading)
{
    ...
    {
        user->monitorRecords.push_back(new MonitorRecord(std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()), reading));        
        if(reading < 2.8)
        {
            //email notification
        }
        ...
        user->medicationRecords.push_back(new MedicationRecord(std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()), dose));
